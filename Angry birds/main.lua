...
function love.load()
    ...
    achievementFont = love.graphics.newFont('flappy.ttf', 11)
...
function love.keypressed(key)
    ...
    if key == 'p' and pause_counter == 0 then
      game_paused = true
      pause_counter = pause_counter + 1
      love.audio.pause(sounds['music'])
      love.audio.play(sounds['pause'])
    elseif key == 'p' and pause_counter % 2 ~= 0 then
      game_paused = false
      pause_counter = pause_counter + 1
...
function love.mouse.wasPressed(button)
  ...
  function love.draw()
      ...
      if game_paused  == true then
        love.graphics.draw(pause, 227, 120, nil, 0.1, 0.1)
        love.graphics.setFont(mediumFont)
        love.graphics.printf("Game paused.\nPress p to resume", 0, 180, VIRTUAL_WIDTH, 'center')
      end
      push:finish()
  end
