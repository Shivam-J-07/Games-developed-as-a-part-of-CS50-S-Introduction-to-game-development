...
function Room:update(dt)
    ...
    for k, object in pairs (self.objects) do
      if object.type == 'pot' and object.beingthrown then
        local potOutOfBounds = false
        ...
        elseif object.y <= MAP_RENDER_OFFSET_Y + TILE_SIZE - object.height / 2 then
          ...
          if object.y + object.height >= bottomEdge then
            potOutOfBounds = true
          end
        end
        if potOutOfBounds then
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 and entity.dead == false then
            ...
            if math.random(4) == 1 then
              entity.givesHealth = true
            end

            if entity.givesHealth then
        ...
        if not entity.dead then
          for k, object in pairs(self.objects) do
            if object.type == 'pot' and entity:collides(object) and object.beingthrown then
              entity:damage(1)
              gSounds['pot-break']:play()
              table.remove(self.objects, k)
            end
