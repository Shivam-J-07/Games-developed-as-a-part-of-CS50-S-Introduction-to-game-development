    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                self:tileSwap(self.highlightedTile, self.board.tiles[y][x], true)
            end
        end
    end
...
function PlayState:tileSwap(tile1, tile2, swap)
  self.board:tileSwap(tile1, tile2)
  ...
  if swap then
    ...
    :finish(function()
      if self.board:calculateMatches() then
        ...
        gSounds['error']:play()
        self:tileSwap(tile1, tile2, false)
...
while self.board:checkForMatches() == false do
    self.board = {}
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
          for k, tile in pairs(match) do
            additionalScore = additionalScore + (tile.variety - 1) * 20
          end
          self.score = self.score + #match * 50 + additionalScore
