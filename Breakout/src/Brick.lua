...
function Brick:hit()
  ...
  gSounds['brick-hit-2']:play()
    ...
    if self.isLocked == false then
      ...
      -- it our se lf.color but with varying alpha; brighter for higher tiers, fading to 0
    ...
    elseif self.isLocked == true and self.unlockable == true then
      self.inPlay = false
      gSounds['brick-hit-1']:stop()
      gSounds['brick-hit-1']:play()
...
function Brick:render()
    if self.inPlay then
        love.graphics.draw(gTextures['main'],
            ...
            gFrames['bricks'][1 + ((self.color - 1) * 4) + self.tier + self.locked],
            self.x, self.y)
    end
end
